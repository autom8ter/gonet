...
func NewGatewayServer(c *config.GrpcConfig) api.Interface {
...
func (s *GatewayServer) Serve(l net.Listener) error {
	...
	grpclog.Infof("grpc-gateway server is starting %v", l.Addr())
...
func (s *GatewayServer) createConn() (conn *grpc.ClientConn, err error) {
	conn, err = grpc.Dial(s.GrpcInternalAddr.Network, s.ClientOptions()...)
	if err != nil {
